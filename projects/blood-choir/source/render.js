  ...
  class Renderer{
    ...
    draw(run,dt,settings,aim=null){
      ...
      for(const e of this.effects){e.life-=dt;if(e.life<=0)continue;const progress=1-e.life/e.max;this.effectAlpha=e.hostile||e.kind==='corpse'?1:fx;c.globalAlpha=Math.min(1,e.life*5)*this.effectAlpha;
        if(e.kind==='crownReward'){this.sprite('crowns',3,4,2,e.x,e.y-progress*24,70,70,false,1-progress);c.font='12px serif';c.textAlign='center';c.fillStyle='#e9c685';if(e.reward)c.fillText('+'+e.reward+' 遺灰',e.x,Math.max(24,e.y-42-progress*30));}
