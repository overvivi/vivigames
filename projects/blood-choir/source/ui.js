(function(){
  ...
  function journeyReport(r){
    ...
    return'<details class="journey-log"><summary>刻んだ道程 <span>'+fmt(journeyView.entries.length+journeyView.omitted)+' 回の選択</span></summary><p>強化・禁忌進化・誓約を選んだ順です。'+(journeyView.omitted?'最初の64件と直近192件を残し、途中 '+fmt(journeyView.omitted)+' 件は省略しています。':'')+'</p><div class="journey-body">'+journeyPageHtml()+'</div></details>';
  }
  ...
  function title(){
    ...
    $('#title-buttons').innerHTML=(checkpoint&&!profile.settled.includes(checkpoint.id)?'<button class="primary" data-action="continue">葬列を続ける <small>WAVE '+checkpoint.wave+'</small></button><button data-action="start">新たな葬送 <small>NEW RUN</small></button>':'<button class="primary" data-action="start">葬送を始める</button>')+'<button data-action="daily">日替わりの悪夢 <small>DAILY RITUAL</small></button>';
  ...
  function dailySummary(date,format=2,history=false){
    ...
    let html='<section class="daily-best" aria-label="日替わりの自己ベスト"><h3>'+date+' の自己ベスト</h3>'+(best?'<div class="daily-stats"><div><strong>'+best.wave+'</strong><small>突破ウェーブ</small></div><div><strong>'+fmt(best.score)+'</strong><small>得点</small></div><div><strong>'+minutes(best.time)+'</strong><small>戦いの時間</small></div></div>':'<p>まだ記録はありません。最初の祈りを刻みましょう。</p>')+'<small>この端末の記録 · 到達ウェーブ、得点、短い時間の順で比較。直近30件を保存</small></section>';
  ...
  function setup(daily){
    ...
    if(daily)content+='<p class="help-copy">日ごとに弔具と仮面が変わります。未解放でも、この試練の中だけで使えます。永久強化は適用されません。同じ日は全員同じ装備・目標・敵編成・強化抽選です。</p>';
    ...
    if(!daily)content+='<div><label class="minor" for="guidance">追い求める禁忌</label><select id="guidance" class="option-select">'+guidanceOptions()+'</select></div></div><div id="guidance-preview" class="guidance-preview" aria-live="polite"></div><p class="minor">3波ごとに、目標の不足材料がひとつ候補に現れます。共鳴は二つの進化を順に導きます。選ぶかは自由です。</p>';
    if(!daily)content+='<details class="seed-options" '+(selectionSeed?'open':'')+'><summary>同じ悪夢に挑む · 種子の指定</summary><label for="run-seed">悪夢の種子（任意）</label><div class="seed-entry"><input id="run-seed" type="text" inputmode="numeric" maxlength="10" placeholder="空欄なら毎回ランダム" aria-describedby="seed-help seed-error"><button type="button" data-action="clearSeed">消す</button></div><p id="seed-help" class="minor">墓碑や結果に残る番号を指定できます。同じ種子・装備・祭壇・深度・選択で、同じ敵編成と強化候補に挑めます。</p><p id="seed-error" role="status" hidden>1〜4294967295の整数を入れてください。</p></details>';
    content+=ritualCard(w.id);
    if(checkpoint)content+='<p class="minor">新しく始めると、保存中の挑戦を終え、獲得済みの遺灰を持ち帰ります。</p>';
    content+='<p class="help-copy">左右移動・空中跳躍・回避で弾を避け、敵を葬ると強化を選べます。初期設定は自動照準・自動射撃。橙色の弾が敵の攻撃です。</p><div class="panel-actions"><button class="primary" data-action="begin" data-daily="'+daily+'">聖堂へ降りる</button><button data-action="training">操作を試す</button><button data-action="trialOptions">弔具を試す</button></div>';
  ...
  function paintGuidancePreview(){
    ...
    target.innerHTML=art(d)+'<div><h3>'+d.name+'</h3><p>'+d.desc+'</p><small>'+(d.pair?'二つの進化が揃うと、自動で共鳴が発現します。':material(d))+'</small></div>'+(d.pair?'<div class="guidance-pair">'+d.pair.map((id,i)=>{const e=C.lookup[id];return'<div>'+art(e)+'<span><b>'+String(i+1).padStart(2,'0')+' · '+e.name+'</b><small>'+material(e)+'</small></span></div>';}).join('')+'</div>':'');
  }
  function resonanceGoalProgress(plan,compact=false){
    return'<div class="resonance-goal '+(compact?'compact':'')+'">'+art(plan.target)+'<div><b>目指す共鳴：'+plan.target.name+'</b><small>'+(plan.blocked?'必要な材料が封印されています。目標の変更も選べます。':plan.complete?'二つの禁忌が重なり、共鳴が発現しました。':plan.steps.some(s=>s.ready)?'条件が揃った進化は候補で選ぶと覚醒します。':plan.active?'いま導く進化：'+plan.active.definition.name:'')+'</small></div><div class="resonance-goal-steps">'+plan.steps.map(step=>'<span class="'+(step.awakened?'awakened':step.blocked?'blocked':step.ready?'ready':'')+'">'+step.definition.name+'<b>'+(step.awakened?'覚醒済み':step.blocked?'材料が封印中':step.ready?'進化を選べます':'材料あと '+step.missing+' 段階')+'</b></span>').join('')+'</div></div>';
  }
  function resonancePaths(){
    return'<h3 class="section-label">二つの禁忌を、ひとつの目標に。</h3><p class="minor">共鳴を選ぶと、一方の材料が揃うたび、もう一方へ導きが移ります。進化の取得は候補で選びます。</p><div class="resonance-path-grid">'+D.resonances.map(d=>{const plan=run.guidancePlan(d.id),selected=run.guidance===d.id,allowed=!run.daily&&!run.practice&&['playing','upgrade','victory'].includes(run.state);return'<section class="item-tile resonance-path '+(selected?'path-target':'')+'">'+resonanceGoalProgress(plan,true)+'<p>'+d.desc+'</p>'+(allowed&&!plan.complete?'<button class="path-choice" data-action="guide" data-id="'+d.id+'" '+(selected||plan.blocked?'disabled':'')+'>'+(selected?'現在の目標':plan.blocked?'必要な能力が封印中':'この共鳴を目指す')+'</button>':'')+'</section>';}).join('')+'</div><h3 class="section-label">一つの進化を目指す</h3>';
  }
  ...
  function covenantAdvice(id){return{glass:'生命の下限は25。すでに傷ついていても、現在生命は新しい上限を超えた分だけ減ります。',hunger:'生命が減っている時の撃破で回復。吸血とは別に働きます。',vigil:'雷・裂け目・鎌に作用します。通常の射撃や亡霊の間隔は変わりません。',chalice:'大奇跡の必要数は8まで。すでに溜めた力は新しい必要数の範囲で保持します。',marrow:'軽減の上限は72%。通常移動は遅くなりますが、回避中の速さは変わりません。',pilgrim:'これから現れる雑魚が対象。主の生命は増えません。遺灰の倍率には他の効果も加わります。'}[id]||'';}
  function covenantPreview(id){
    ...
    let hint=covenantAdvice(id);if(id==='vigil'&&!run.stats.lightning&&!run.stats.void&&!run.stats.cleave)hint='今は対象の自動スキルを持っていません。雷・裂け目・鎌を得ると力を発揮します。';
    return'<span class="covenant-delta" aria-label="誓約による実際の変化">'+rows.join('')+'</span><span class="covenant-note">'+hint+'</span>';
  }
  function covenant(){
    const html=header('A COVENANT IN BLOOD','遺骸に、誓う。','ボスごとに一度だけ。代償はこの挑戦が終わるまで続く。')+'<p class="covenant-intro">現在の装備・能力で、誓った直後の変化を比べています。</p><div class="selection-grid covenant-grid">'+run.covenantChoices.map(id=>{const d=D.covenants.find(d=>d.id===id);return'<article class="upgrade-card"><button class="choose" data-action="swear" data-id="'+id+'"><span class="school-label">血 の 誓 約</span>'+art(d)+'<h3>'+d.name+'</h3><p>'+d.desc+'</p>'+covenantPreview(id)+'</button></article>';}).join('')+'</div><div class="panel-actions"><button data-action="backToRun">今は誓わない</button></div>';show('covenant',html);
  }
  function pause(){
    if(run?.trial){show('pause',header('THE QUIET ARMORY','試射を、ひと休み。',run.weapon.name+' × '+run.mask.name,false)+'<p class="help-copy">標的は何度でも戻ります。通常の挑戦・遺灰・実績は変わりません。</p><div class="panel-actions"><button class="primary" data-action="backToRun">試射を続ける</button><button data-action="trialOptions">'+(run.trialRecord?'標的の設定':'組み合わせを変える')+'</button><button data-action="trialNotes">計測の控え</button><button data-action="build">能力を見る</button><button data-action="settings">設定</button><button data-action="leaveTrial">試射を終える</button></div>',true);return;}
  ...
  function weaponLedger(){return'<section class="weapon-ledger"><h3 class="section-label">六つの弔具の、足跡</h3><p class="minor">帰還した挑戦の最深記録です。深度と日替わりを通じて残り、古い墓碑が一覧から外れても失われません。追憶や試射は含みません。</p><div>'+D.weapons.map(d=>'<article>'+art(d)+'<div><h4>'+d.name+'</h4>'+weaponMilestones(d.id)+'</div></article>').join('')+'</div></section>';}
  function loadout(){
  ...
  function medalGains(){
    ...
    return'<section class="medals-earned"><h3 class="section-label">新たな討伐の記章</h3><div>'+run.medalsEarned.map(m=>'<article><canvas data-bestiary="'+(m.tier-1)+'" width="128" height="128" aria-hidden="true"></canvas><p><b>'+[...D.enemies,...D.bosses].find(d=>d.id===m.id).name+'</b><span>'+m.label+'</span></p></article>').join('')+'</div><p class="minor">禁書の「異形」に、討伐の足跡を残しました。</p></section>';
  }
  function codex(){
    ...
    if(codexTab==='covenants'){
      html+='<section class="covenant-guide"><p>主を葬った後、三つの候補から一度だけ誓えます。強さと代償は帰還まで残り、同じ誓約を重ねることはできません。</p><p>選ばずに進むこともできます。実際に誓う前の画面では、その時の装備・能力に合わせた変化を表示します。</p></section>';
    ...
    if(codexTab==='achievements'){
      html+='<div class="achievement-guide"><p>帰還して記録した進行です。狙う実績を選ぶと、扉の画面へ飾れます。</p>'+(profile.achievementGoal?'<button data-action="clearAchievementGoal">目標を自動で選ぶ</button>':'')+'</div>';
  ...
  function rememberControl(key,r){
    ...
    return'<div class="remember-control"><button data-action="remember" data-id="'+key+'" '+(!kept&&full?'disabled':'')+'>'+(kept?'この墓碑を保存枠から外す':full?'保存枠が埋まっています':'この墓碑を手元に残す')+'</button><p>'+(kept?'手元の墓碑に保存済み。上位記録が入れ替わっても残ります。':'手元の墓碑 '+profile.keptRecords.length+' / 6。残したビルドは後から追憶や再挑戦に使えます。')+'</p></div>';
  }
  ...
  function keptRecords(){
    ...
    return'<section class="kept-records"><h3 class="section-label">手元に残した墓碑 <span>'+profile.keptRecords.length+' / 6</span></h3><p class="minor">忘れたくないビルドを、上位30件の記録とは別に残せます。</p>'+(profile.keptRecords.length?'<div>'+profile.keptRecords.map((r,i)=>{const w=D.weapons.find(d=>d.id===r.weapon),m=D.masks.find(d=>d.id===r.mask);return'<button data-action="recordDetail" data-id="kept:'+i+'" aria-label="手元の墓碑 '+(i+1)+' の詳細">'+art(w)+'<span><b>'+w.name+' × '+m.name+'</b><small>第'+r.wave+'波 · 禁忌進化 '+r.evolved.length+' 種<br>'+(r.daily?'日替わり':'深度 '+(r.difficulty+1))+' · '+fmt(r.score)+' 点</small></span><i>↗</i></button>';}).join('')+'</div>':'<p class="empty-memory">墓碑の詳細か葬送の結果から「手元に残す」を選べます。</p>')+'</section>';
  }
  function records(){
  ...
  function recordDetail(index){
    ...
    html+='<p class="progress-line">最長連祷 '+r.bestCombo+' 体</p><p class="progress-line">'+(r.seed?'悪夢の種子 '+r.seed+' · 当時の装備・祭壇・深度で最初から再挑戦できます。強化と誓約は選び直します。':'この古い墓碑には、再挑戦の条件が残っていません。')+'</p>';
    if(checkpoint&&r.seed)html+='<p class="minor">再挑戦すると、中断している葬送を終了し、獲得済みの遺灰を受け取ります。</p>';
  ...
  function practiceOptions(index){
    ...
    const chain='<div id="practice-chain" hidden>'+practiceRoute(waves)+'<p class="help-copy">最初の主から順に、生命100%の主へ挑みます。自分の生命・結界・大奇跡・復活の残りは次戦へ引き継ぎます。幕間に生命20%回復／大奇跡35%補充／結界18の一つを選べます。</p></div>';
    show('practiceOptions',header('THE CHAMBER OF REMEMBRANCE','追憶の祭室','葬ったビルドで、もう一度あの主と。')+'<p class="help-copy">'+D.weapons.find(d=>d.id===record.weapon).name+' × '+D.masks.find(d=>d.id===record.mask).name+' · 深度 '+(record.difficulty+1)+'<br>墓碑の強化・進化・誓約・成長を再現し、出発時に生命・結界・大奇跡・復活を補充します。主と、その召喚敵に挑みます。</p>'+form+single+chain+'<p class="minor">遺灰・実績・墓碑の更新はありません。通常の中断記録は保持されます。追憶は途中保存されません。</p><div class="panel-actions"><button class="primary" data-action="startPractice">記憶へ降りる</button><button data-action="recordDetail" data-id="'+index+'">墓碑へ戻る</button></div>',true);
  ...
  function help(){
    show('help',header('HOW TO SURVIVE','生き残るために','')+'<div class="help-grid"><p><kbd>A</kbd><kbd>D</kbd> / ← →<br>左右へ滑るように移動</p><p><kbd>SPACE</kbd> / W / ↑<br>二段跳躍。空中でもう一度</p><p><kbd>SHIFT</kbd> / 右クリック<br>短い無敵回避。移動方向へ</p><p><kbd>E</kbd><br>撃破で溜まる大奇跡。敵弾を一掃</p><p><kbd>ESC</kbd><br>一時停止 / ビルド確認</p><p>マウスで照準、長押し射撃<br>自動照準・射撃は設定で切替</p></div><h3 class="section-label">敵を葬り、禁忌を選べ</h3><p class="help-copy">4.5秒以内に敵を倒し続けると「連祷」が続き、5体ごとに撃破得点の倍率が上がります（最大2倍）。生命を削られると途切れます。結界で防げば継続。<br><br>橙色の弾は敵の攻撃。足元の小さな輪が自分の当たり判定、中央の点がその中心です。背景の暗さ、輪の表示、味方演出の濃さ、被弾・無敵の点滅は設定で変更できます。全ての敵を倒すと強化をひとつ選びます。同じ強化を重ね、禁書に記された2種の条件を満たすと「禁忌進化」が候補に現れます。特定の2つの進化を同じ挑戦で揃えると、さらに「共鳴」が自動で発現します。組み合わせは禁書に記されています。<br><br>8波ごとに聖堂の主が待ちます。第32波で神を葬ると、帰還か無限の葬送を選べます。倒れても遺灰と発見は残ります。遺灰で弔具・仮面・祭壇を解放しましょう。第9波以降の一部の波には「冠の異形」が現れます。冠を砕くと追加の遺灰を得ます。<br><br>墓碑の詳細から過去のビルドを振り返り、到達済みの主に挑む「追憶の祭室」へ進めます。第15波突破後は主を巡る連戦も選べます。生命や大奇跡を持ち越し、幕間の供物を一つ選んで進みましょう。追憶は通常の中断記録と報酬を変えません。<br><br>標準配置のコントローラーにも対応。左スティック/十字キーで移動、下ボタン（A/×）で跳躍・決定、右ボタン（B/○）で回避・戻る、左ボタン（X/□）で大奇跡、STARTで休息。右スティックを倒している間は手動照準、それ以外は自動です。メニューは十字キーで選び、深度などは左右で変更できます。<br><br>スマホは横持ち推奨。画面下の移動・跳躍・回避を使い、自動照準と自動射撃で戦えます。中断した挑戦は、そのウェーブの最初から再開します。</p><div class="panel-actions">'+(!run?'<button class="primary" data-action="training">操作を試す</button>':'')+'<button data-action="close">わかった</button></div>',true);
  }
  ...
  function recordTrialOptions(){
    ...
    show('trialOptions',header('THE MEMORY IN YOUR HANDS','墓碑のビルドを試す','第'+r.wave+'波 · '+w.name+' × '+m.name)+'<p class="help-copy">あの葬送で刻んだ力を、そのまま手に。強化・進化・誓約・祭壇・成長を再現します。通常の挑戦・遺灰・実績と、この墓碑は変わりません。</p><div class="record-trial-summary">'+art(w)+'<div><h3>'+w.name+' × '+m.name+'</h3><p>強化 '+levels+'段階 · 禁忌進化 '+r.evolved.length+'種 · 誓約 '+r.covenants.length+'種</p><p>祭壇 '+meta+'段階 · 成長による威力 +'+Number((r.growth*100).toFixed(1))+'%</p></div></div><div class="trial-target-setting">'+targetSelect(trialRecord.targetHp)+'</div>'+resonanceSection(r.evolved)+'<p class="minor">標的は一秒後に戻り、大奇跡は五秒で再充填。損傷は標的の残り生命を超えた分を含めず、直近10秒の合計を10で割って表示します。生命を変えると計測をやり直します。被弾しないため、瀕死時や復活時の効果はこの場では発動しません。</p><div class="panel-actions"><button class="primary" data-action="startTrial">このビルドで試す</button><button data-action="freeTrial">自由な組み合わせへ</button><button data-action="leaveTrial">墓碑へ戻る</button></div>',true);
  }
  function trialOptions(){
    ...
    show('trialOptions',header('THE QUIET ARMORY','弔具の試射室','解放前の装備も、ここでは手に取れる。')+'<p class="help-copy">三つの標的を相手に、射撃・移動・大奇跡を試せます。傷つかず、標的は何度でも戻ります。通常の挑戦・遺灰・実績は変わりません。</p><div class="trial-options">'+select('trial-weapon','試す弔具',D.weapons,opts.weapon)+select('trial-mask','試す仮面',D.masks,opts.mask)+select('trial-rite','刻む禁忌',[{id:'none',name:'初期状態 — 弔具と仮面だけ'},...D.evolutions,...D.resonances.map(d=>({...d,name:'共鳴 — '+d.name}))],opts.rite)+targetSelect(opts.targetHp||600)+'</div><div id="trial-preview"></div><p class="minor">祭壇・誓約・成長はなし。禁忌を選ぶと、その進化に必要な強化を刻みます。標的の生命は600・6,000・60,000から選べます。強い構成は高い生命で試すと、余剰分を切り捨てられにくくなります。損傷は残り生命を超えた分を含めず、直近10秒の合計を10で割って表示します。</p><div class="panel-actions"><button class="primary" data-action="startTrial">この組み合わせで試す</button><button data-action="leaveTrial">弔具と仮面へ戻る</button></div>',true);
  ...
  function captureTrial(index){if(!trial||run!==trial.run||![0,1].includes(index))return;const note=trial.snapshot();if(!note){toast('十秒ほど試射してから、計測を控えられます。');return;}trialNotes[index]=note;showTrialNotes();}
  function showTrialNotes(){
    if(!trial||run!==trial.run)return;const ready=run.time>=10,one=n=>Number(n.toFixed(1)),names=['A','B'];
    let content=header('TWO ECHOES OF THE SAME PRAYER','計測の控え','弔具や禁忌を替え、傷の違いを見比べる。')+'<p class="help-copy">直近10秒の実損傷を、二つまで控えます。大奇跡や移動の回数でも数値は変わります。ページを再読み込みすると控えは消え、通常の記録には残りません。</p>';
    if(trialNotes.every(Boolean)){const [a,b]=trialNotes,delta=b.dps-a.dps;content+='<div class="trial-comparison"><b>B − A　'+(delta>0?'+':'')+one(delta)+' 損傷/秒</b><span>'+(a.targetHp===b.targetHp?'標的の生命は同じ '+fmt(a.targetHp)+'。同じ操作で比べると、構成の差を確かめやすくなります。':'標的の生命が異なります。倒し切った時の余剰分が変わるため、同じ生命で計測し直すと比較しやすくなります。')+'</span></div>';}
  ...
  function victory(){
    ...
    show('victory',header('THE GOD IS SILENT','神は、ようやく死んだ。','',false)+'<div class="victory-scene">'+image+summary+'</div><div class="panel-actions"><button class="primary" data-action="endless">さらに深く — 無限の葬送</button><button data-action="finish">帰還する</button></div><p class="progress-line">帰還で次の深度を解放。無限挑戦を続けて倒れてもクリアは記録されます。第64波には、まだ鳴り止まない弔鐘が待っています。</p>');
  }
  ...
  function results(){
    ...
    html+='<p class="progress-line">悪夢の種子 '+run.seed+' · 同じ種子・装備・選択で、同じ敵編成と強化候補に挑めます。</p><div class="panel-actions"><button class="primary" data-action="retry">もう一度、葬送へ</button><button data-action="sameSeed">同じ悪夢に再挑戦</button><button data-action="title">扉へ戻る</button></div>';
    show('results',html);
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(a==='abandonConfirm')show('abandon',header('RETURN TO THE SURFACE','ここで、葬送を終える。','',false)+'<p class="help-copy">現在の挑戦を終了して、獲得済みの '+fmt(run.souls)+' 遺灰を持ち帰ります。</p><div class="panel-actions"><button class="primary" data-action="finish">帰還する</button><button data-action="backToRun">まだ戦う</button></div>',true);
  ...
  function qaAction(a){
    ...
    if(a==='qaPhase'){const boss=run.enemies.find(e=>e.boss&&!e.dead);if(boss){boss.hp=boss.maxHp*.44;resume();toast('ボスの第二段階を検証');}else toast('ボスが出現してから使ってください。');}
